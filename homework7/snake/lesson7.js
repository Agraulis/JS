...
function startGame() {
    ...
    barrier_timer = setInterval(createBarrier, BARRIER_SPEED);
}
...
function move() {
    ...
    if (new_unit == undefined) {
        new_unit = document.getElementsByClassName('cell-' + (direction[0] == 'x' ? coord_y : (direction[1] == '+' ? FIELD_SIZE_Y - 1 : 0)) + '-' + (direction[0] == 'y' ? coord_x : (direction[1] == '-' ? FIELD_SIZE_X - 1 : 0)))[0];
    }

    if (!isSnakeUnit(new_unit) && !new_unit.getAttribute('class').split(' ').includes('barrier-unit')) {
